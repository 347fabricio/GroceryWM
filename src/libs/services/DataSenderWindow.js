...
class DataSenderWindow {
  createNewProduct() {
    document.querySelector("#btnSubmit").addEventListener("click", async () => {
      const { isEmpty, productDetails } = this.getEmpty("post");

      if (!isEmpty) {
        let currentPage = document.querySelector("#products").getAttribute("page");
        apiConnector.post(productDetails, --currentPage);
        this.cleanInputs();
      }
    });
  }
  ...
  async updateProduct(id) {
    document.querySelector("#btnSubmit").addEventListener("click", async () => {
      const { isEmpty, productDetails } = this.getEmpty("update", id);

      if (!isEmpty) {
        let currentPage = document.querySelector("#products").getAttribute("page");
        apiConnector.put(id, productDetails, --currentPage);
        document.querySelector("#dSWindow").classList.remove("d-block");
        document.querySelector("#dSWindow").classList.add("d-none");
        windowUtils.visibilityOn();
        windowUtils.unlockContent();
      }
    });
  }
  ...
  cleanDateInput(element, index) {
    for (let j = 0; j < element[index].children.length; j++) {
      element[index].children[j].value = "";
    }
  }
  ...
  getEmpty(method, id) {
    ...
    const productDetails = this.setValues(inputs, method, id);
  ...
  setValues(inputs, method, id) {
    let productDetails = new ProductDetails();

    method == "post" ? (productDetails.id = ID++) : (productDetails.id = id);
